...
for i, file in enumerate(all_files[strt:]):
    print(i + strt)
    ...
    if (rpeaks['ECG_R_Peaks'].any()):
        filtered_rpeaks = list(
            filter(lambda r: (r > 250 or r < 28250), rpeaks["ECG_R_Peaks"]))  # dwt fails when R is close to 0

        _, waves_peak = nk.ecg_delineate(cleaned, rpeaks, sampling_rate=250, show=False, method='peaks',
                                         show_type='all')
        ...
        hrv = nk.hrv(peaks, sampling_rate=250)

        # with pd.option_context('display.max_columns', 40):
        #    print(wvpk.head())

        df_hrv = pd.DataFrame.from_dict(hrv)
        df_hrv.insert(0, 'Time', date)

        a = tpot_tgt[abs(tpot_tgt['date'] - date) < datetime.timedelta(seconds=149)]
        a = a.iloc[0]

        df_hrv['glucose'] = a['glucose']
        df_hrv['target'] = a['target']

        # boxplot = wvpk.boxplot()
        # plt.show()
        date_time = file.replace('flt_ECG', 'feat')
        date_time = date_time.replace('.csv.csv', '')

        df_hrv.to_csv(fr'C:/Users/nrust/Downloads/D0_test/%s_hrv/%s_%s.csv' % (participant, participant, date_time), index=False)
