...
ECGFeat = pd.DataFrame(FeatECG, columns=['date', 'P_mean', 'P_std', 'Q_mean', 'Q_std', 'S_mean', 'S_std', 'T_mean', 'T_std',
...
for i, file in enumerate(all_files[:]):
    ...
    if (np.median(cleaned**2) < 80 and np.std(cleaned**2) < 2500):
        plt.plot(cleaned)
        print(np.median(cleaned**2), np.std(cleaned**2))
        plt.show()
...
def plot_results():
    ...
    df2 = pd.DataFrame(values,
                       columns=['date', 'P_mean', 'P_std', 'Q_mean', 'Q_std', 'S_mean', 'S_std', 'T_mean', 'T_std', 'QT_mean',
    ...
    #result.to_csv(fr'C:\Users\nrust\Downloads\D0_test\005_feat\ecg_feat_all.csv', index=False)